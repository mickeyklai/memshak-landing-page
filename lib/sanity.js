...
function getSanityClient(options = {}) {
    ...
    if (!projectId || String(projectId).trim() === '') {
        throw new Error(
            'SANITY_PROJECT_ID is not set. In Netlify: Site configuration → Environment variables → add SANITY_PROJECT_ID (same public ID as in Sanity). Locally: root .env or `netlify dev` env.',
        );
    }
